...
-module(lager_iowrite_formatter).
-author('Łukasz Niemier <lukasz.niemier@kobil.com>').
...
output({Format, Prop, Default}, Msg) when is_list(Format) and is_atom(Prop) ->
    %% sort of like a poor man's ternary operator
    Metadata = lager_msg:metadata(Msg),
    case get_metadata(Prop, Metadata) of
        undefined ->
            make_printable(output(Default,Msg));
        Value ->
            make_printable(io_lib:format(Format, [Value]))
    end;
...
basic_test_() ->
    ...
    [{"Default formatting test",
        ...
        {"Metadata can have extra formatting4",
            ?_assertEqual(iolist_to_binary(["(deadbeef)"]),
                iolist_to_binary(format(lager_msg:new("Message",
                            Now,
                            error,
                            [{pid, 16#deadbeef}],
